# docker build -t my-neovim-dev:v0.0.1 -f my-neovim-dev_amd64.dockerfile .
...
ENV UBUNTU_DEFAULT_MIRROR=http://archive.ubuntu.com
...
ARG HOSTS_FILE
ARG GIT_HUB_MIRROR_URL=""

SHELL ["/bin/bash", "-c"]

# æ~[¿æ~M¢ /etc/hosts æ~V~G件
COPY $HOSTS_FILE /etc/hosts

# Replace mirror in sources.list
RUN sed -i "s|${UBUNTU_DEFAULT_MIRROR}|${UBUNTU_MIRROR}|g" /etc/apt/sources.list

# Update apt again after changing mirror
